...
class Main(Star):
    ...
    def _get_user_data(self, group_id: str, user_id: str) -> Dict:
        ...
        if user_id not in group_data:
            group_data[user_id] = {
                ...
                "loan_amount": 0,  # 欠款总额
                "last_loan_interest_time": int(time.time()),  # 上次计息时间
                "jailed_until": 0,
    ...
    def _update_loan_interest(self, user_data: Dict) -> int:
        ...
        if hours >= 1:
            # 计算复利：新债务 = 旧债务 * (1 + 利率)^小时数
            new_loan = int(loan * ((1 + rate) ** hours))
            interest_added = new_loan - loan
    ...
    async def bank_info(self, event: AstrMessageEvent):
        ...
        async with session_lock_manager.acquire_lock(f"pet_market_{group_id}_{user_id}"):
            ...
            yield event.plain_result(
                ...
                f"💰 存款余额：{bank} / {limit}\n"
                f"📈 存款利息：{rate * 100}%/小时\n"
                f"💵 待领利息：{potential_interest} 金币\n"
                f"----------------------\n"
                f"💸 当前欠款：{loan} / {loan_limit}\n"
                f"📉 贷款利息：{loan_rate * 100}%/小时\n"
                f"----------------------\n"
                f"⬆️ 升级费用：{next_cost} 金币\n"
                f"💡 提示：信用等级越高，存款上限和贷款额度越高。"
            )
    ...
    async def upgrade_bank(self, event: AstrMessageEvent):
        ...
        async with session_lock_manager.acquire_lock(f"pet_market_{group_id}_{user_id}"):
            ...
            new_loan_limit = self._get_loan_limit(user["bank_level"])

            yield event.plain_result(
                f"✅ 升级成功！信用等级提升至 Lv.{user['bank_level']}\n"
                f"📦 新存储上限：{new_limit} 金币\n"
                f"💳 新贷款额度：{new_loan_limit} 金币\n"
    ...
    async def bank_interest_rate(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(
            f"【💹 银行利息说明】\n"
            f"📈 存款利率：{rate * 100}%/小时\n"
            f"📉 贷款利率：{loan_rate * 100}%/小时\n"
            f"🔄 计息方式：复利\n"
            f"⏰ 存款最大计息时间：{max_hours} 小时\n"
            f"⏰ 贷款无最大时间限制，持续计息！\n"
        )
    ...
    # ==================== 命令：贷款 ====================
    @filter.command("贷款")
    async def take_loan(self, event: AstrMessageEvent, amount: int):
        """向银行贷款"""
        group_id = str(event.message_obj.group_id)
        user_id = str(event.get_sender_id())

        if amount <= 0:
            yield event.plain_result("❌ 贷款金额必须大于 0。")
            return
        ...
        async with session_lock_manager.acquire_lock(f"pet_market_{group_id}_{user_id}"):
            ...
            # 1. 先结算之前的利息
            added_interest = self._update_loan_interest(user)
            ...
            if current_loan + amount > limit:
                ...
                msg = f"❌ 贷款被拒绝！你的信用额度不足。\n"
                if added_interest > 0:
                    msg += f"📉 刚刚产生了 {added_interest} 金币的利息。\n"
                msg += f"当前欠款：{current_loan}\n"
                msg += f"信用额度：{limit}\n"
                msg += f"剩余可贷：{can_borrow}"
                self._save_user_data(group_id, user_id, user)  # 保存利息更新
            ...
            if added_interest > 0:
                msg += f"📉 (包含结算此前产生的 {added_interest} 利息)\n"
            msg += f"💸 当前欠款：{user['loan_amount']} 金币\n"
    ...
    async def repay_loan(self, event: AstrMessageEvent, amount: Optional[int] = None):
        ...
        async with session_lock_manager.acquire_lock(f"pet_market_{group_id}_{user_id}"):
            ...
            if user_coins < real_repay:
                msg = f"❌ 余额不足！\n"
                msg += f"你需要还款：{real_repay} 金币\n"
                msg += f"当前余额：{user_coins} 金币"
                self._save_user_data(group_id, user_id, user)
                yield event.plain_result(msg)
                return
            ...
            if added_interest > 0:
                msg += f"📉 (包含结算此前产生的 {added_interest} 利息)\n"
            msg += f"💸 剩余欠款：{user['loan_amount']} 金币\n"
            msg += f"💵 当前余额：{user['coins']} 金币"
    ...
    async def total_rich_ranking(self, event: AstrMessageEvent, page: int = 1):
        ...
        ranked = sorted(
            pets.items(),
            key=lambda x: x[1].get("coins", 0) + x[1].get("bank", 0) - x[1].get("loan_amount", 0),
            reverse=True
        )
        ...
        for i, (uid, data) in enumerate(ranked[start:end], start + 1):
            ...
            total_assets = coins + bank - loan
            medal = ["🥇", "🥈", "🥉"][i - 1] if i <= 3 else f"{i}."
            lines.append(f"{medal} {name} - {total_assets} 金币")
        ...
        async with session_lock_manager.acquire_lock(f"pet_market_{group_id}_{lock_ids[0]}"):
            async with session_lock_manager.acquire_lock(f"pet_market_{group_id}_{lock_ids[1]}"):
                ...
                if random.random() < success_rate:
                    ...
                    user_data["jailed_until"] = int(time.time()) + 86400
                    self._save_user_data(group_id, user_id, user_data)
